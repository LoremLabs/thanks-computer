<script lang="ts">
    ...
    // Vendored from admin-ui's StackView — the canvas op-stack diagram.
    // The admin version also hosts a "+ Add op" form (store + NewOpForm);
    // the demo authors via its editor, so that affordance is
    // dropped and only the renderer remains.
    interface Props {
        stack: string
        ops: Op[]
        lastDurations: Record<string, number>
        // Time spent in this stack on the most recent trace that
        // touched it — sum of step.duration_ms across this stack's
        // steps. Rendered next to the "output" label. Undefined when
        // no trace data is available.
        stackTotalMs?: number
        // Optional click handler on an op box.
        onSelectOp?: (op: Op) => void
        // opId of the currently-selected op — its box is drawn with a
        // highlighted border. Undefined = no selection highlight.
        selected?: string
    }

    let { stack, ops, lastDurations, stackTotalMs, onSelectOp, selected }: Props =
        $props()
    ...
    function render() {
        ...
        for (const row of myGroup.scopes) {
            ...
            for (let i = 0; i < n; i++) {
                ...
                const isSel = selected != null && opId(op) === selected
                ctx.fillStyle = '#ffffff'
                ctx.strokeStyle = isSel ? '#06b6d4' : '#a3a3a3'
...
<div bind:this={containerEl} class="h-full w-full overflow-auto">
    <div class="px-1 py-2 sm:px-2 sm:py-3">
        <header class="mb-2 flex items-center gap-2 px-1 sm:px-2">
