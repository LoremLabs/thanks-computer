...
The Thanks Computer Language (TXCL) is a domain specific language which controls when our operators will execute.
...
The smallest resonator emits a value into the flow:
...
```txcl
WHEN .tz == "ams" EXEC "https://timeapi.io/api/v1/time/current/zone?timezone=Europe%2FAmsterdam"
```
...
A signed integer. When multiple resonators at the same stage match, the highest-priority resonator wins selection. Default is 0.
