...
1. **Bootstrap once per chassis**: mint a master-key file, point the
   chassis at it, restart.
2. **Manage secrets via CLI**: `txco auth tenant secrets {set,
   generate, list, show, rotate, revoke}`. Operator-supplied values
   come from a TTY prompt; chassis-generated values are printed
   exactly once.
...
| `secret:*:write` | Create, generate, rotate, rotate-generated, patch description, revoke. |

There is **no** `secret:*:reveal` in v1. Per design §5, revealing is
itself a leak surface; rotate the secret to see a new value once.
...
## 7. What's NOT supported in v1

The following are deliberately out of scope:

- **Reveal endpoint or capability.** No way to read a stored value
  back. Rotate to see a new value once.
- **Rename.** Names are immutable. Rename = create-new + revoke-old.
- **Online master-key rotation.** `txco auth secrets master-rotate`
  prints "not yet implemented; schema is ready". Phase 2.
- **HSM / cloud KMS-backed master keys.** Interface seam exists in
  `chassis/secrets/crypto.go` (`MasterKeyProvider`); an alternative
  implementation can be registered behind it.
- **Cleartext-out-of-process transports.** v1 supports only the
  in-process sealed bag — cleartext on a separate worker is a
  Phase-2 follow-up.
- **Cross-tenant secret sharing.** Tenants are isolated by design.
- **Admin UI for secrets.** v1 surface is CLI-only (matches the
  existing pattern — hostnames is also CLI-only). A "tenant admin
  UI" is a separate future arc.
...
| `secret:*:write` | set, generate, rotate, rotate-generated, describe, revoke |
