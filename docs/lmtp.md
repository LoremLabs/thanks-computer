...
The `lmtp` personality is a chassis head that speaks LMTP (RFC 2033) to a colocated Postfix (or any LMTP client). Inbound mail becomes a normal event envelope (`_txc.src=lmtp`, `_txc.lmtp.*`) so resonators can SELECT on recipient, subject, headers, body, attachments, and emit per-recipient verdicts back to Postfix.
...
**Enabled by `--personalities`** — the personality string is the only gate. Once `lmtp` is in the list, the head binds `:2424` by default. A chassis upgrade that doesn't change `--personalities` cannot acquire an LMTP listener.
...
**TCP LMTP has no built-in authentication in v1.** Restrict access at the network layer (security group, WireGuard tunnel, firewall) — `--lmtp-listen-addrs=10.0.0.5:2424` plus an ingress allowlist. SASL between Postfix and the chassis is a Phase 5 follow-up.
...
The rule still SELECTs / EXECs against the full envelope; only what hits disk is masked. Scope is per-`(tenant, stack)` and hints take effect at the next dbcache reload. See [`docs/txcl.md` § WITH](txcl.md#with--per-call-modifiers) for the full semantic.

## Out of scope (deferred)

- **Outbound mail.** This personality is inbound only. Outbound (`txco://mail-send` op + DKIM signing + deliverability tooling) is a separate, larger story.
- **Public SMTP listener.** The chassis takes LMTP from a trusted Postfix; it doesn't accept SMTP on the open internet.
- **STARTTLS termination.** Postfix terminates TLS at the edge. Inside the box LMTP is plaintext over Unix socket.
- **DKIM/SPF/DMARC verification.** Postfix does this in front of us and stamps `Authentication-Results`. The chassis reads but does not verify.
- **Sieve dialect.** txcl already covers this vocabulary; no value in supporting both.
- **Built-in Maildir delivery.** Final delivery is an op (forward to S3, post to a ticketing API, write to a mailbox-as-a-service); the chassis is a router, not a mail store.
- **Per-tenant size overrides, large-attachment offload, DB-backed recipient routing, recipient-ownership verification.** All Phase 5 polish.

## See also

- [`docs/ingress.md`](ingress.md) — the ingress router this builds on.
