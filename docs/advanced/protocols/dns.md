...
| Contacts clients | **SRV** `_carddavs._tcp` + **TXT** `path=/.well-known/carddav` (RFC 6764) at the same owners when `--dns-carddavs-port` is set, for clients that discover the [contacts](./contacts.md) server from an address |
