...
| `txco://drive/collection` · `drive/account` · `drive/put` · `drive/get` · `drive/stat` · `drive/list` · `drive/delete` · `drive/mkdir` · `drive/move` · `drive/copy` | A mutable document space — folder trees a client mounts over WebDAV (the `webdav` personality) and a stack writes, reads, lists (`since` a sync token, deletions included), moves and copies; a resource keeps its `resource_id` across renames and every mutation is a `_scheduled` event. See [drive](./drive.md). |
