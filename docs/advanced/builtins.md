...
| `txco://kv/get` · `kv/set` · `kv/delete` · `kv/incr` · `kv/cas` · `kv/mget` · `kv/list` | Read + write durable state across requests — counters, flags, locks, caches (`boltdb` local / `redis` shared); read many keys across namespaces in one dispatch, or list a namespace a sorted page at a time, with values. See [kv](./kv.md). |
