...
| state | `@state.{machine,id,from,to,version,event_id,attempt}`, `@state.cause.{source,stack,trace,run}` — a committed state transition, read-only; see [state](./protocols/state.md) |
