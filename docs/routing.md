...
| `tcp`  | The connection hostname (TLS SNI on a `;tls` listener, or a trusted edge's report of it on a `;proxy=` one) against **verified** tenant hostnames first — routing into that hostname's stack's inlet for the listener's protocol (`<stack>/_tcp`; `<stack>/_NAME` on a `;handler=NAME` listener), and only if the stack has an active, non-empty one (TCP is opt-in per stack, per protocol); then the listener name (from `--tcp-listen-addrs`, `name=addr` form; bare addrs use `default`) against `tcp.listeners`. No match closes the connection. | `_txc.tcp.host`, `_txc.tcp.inlet`, `_txc.tcp.listener` |
