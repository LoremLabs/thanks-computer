...
`--trace-dir` is the root (default `./traces`). `--trace-mode=full --trace-dir=/var/log/txco/traces` is a typical production setting when you actually want payloads.
...
txco trace <rid> --verbose  # include payloads (ModeFull only)
```
...
- Paths are exact gjson dot-paths. No wildcards or regex in v1.
