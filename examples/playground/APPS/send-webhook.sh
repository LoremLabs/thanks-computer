...
#
# In the playground the stripe stack is reached via the /stripe/* path
# (see OPS/_sys/boot/25/route-stripe.txcl), so the default URL posts there.
#
# Usage:
#   STRIPE_WEBHOOK_SECRET=whsec_demo_secret ./send-webhook.sh
#   ./send-webhook.sh http://localhost:8080/stripe/webhooks   # custom URL
#
# Requires: bash, curl, openssl.
set -euo pipefail

SECRET="${STRIPE_WEBHOOK_SECRET:-whsec_demo_secret}"
URL="${1:-http://localhost:8080/stripe/webhooks}"
