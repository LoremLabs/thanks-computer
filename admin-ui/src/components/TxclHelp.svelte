    ...
    {#if alwaysOpen || open}
        <div class="max-h-[50vh] overflow-auto {alwaysOpen ? '' : 'border-t border-neutral-200'} px-4 py-3 text-xs leading-relaxed text-neutral-700">
            <section class="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1">
                ...
                <code class="text-sky-700">LOOP [EVERY &lt;d&gt;] [SET &lt;path&gt; = &lt;value&gt;] UNTIL &lt;predicate&gt; [MAX &lt;n&gt;]</code>
                <span class="text-neutral-500">re-run this rule's EXEC inside one dispatch until the predicate holds against the accumulated view, merging every pass into one payload — the scope waits for the loop. <code class="text-neutral-700">UNTIL</code> is a WHEN condition; <code class="text-neutral-700">MAX</code> defaults to 10, <code class="text-neutral-700">EVERY</code> (the pause between passes) to 50ms; <code class="text-neutral-700">SET</code> runs before each repeat to feed a cursor into the input. WITH values are re-resolved each pass. Ends with <code class="text-neutral-700">@runtime.loop.&lt;op&gt;.stop</code> = done | max | timeout | fuel | halted | error.</span>
