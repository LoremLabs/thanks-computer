...
// existing backends (same posture as calendar.Config / imap.Config).
type Config struct {
	// DBPath is the bundled SQLite backend's file path (--contacts-db-path).
	DBPath string
}
...
// what is available (so a misconfigured --contacts-store fails loudly).
func Open(name string, cfg Config) (*Store, error) {
	...
	if !ok {
		...
		return nil, fmt.Errorf("contacts: unknown store %q (available: %v)", name, avail)
	}
	return c(cfg)
}
