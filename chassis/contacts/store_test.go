...
func TestEnsureSchemaDropsTheOldPasswordColumn(t *testing.T) {
	...
	defer db.Close()
	if _, err := db.Exec(`CREATE TABLE contacts_accounts (
	...
	if _, err := db.Exec(`INSERT INTO contacts_accounts (tenant, username, pw_hash,  created_at, updated_at)
	...
	if _, err := db.Exec(`SELECT pw_hash FROM contacts_accounts`); err == nil {
