// Package apppass is the app-password mechanism the stateful personalities
// share: argon2id hashing in PHC form, constant-time verification, a dummy
// verify that hides account existence, two generators (a grouped token and
// a BIP-39 word phrase), and a verified-login cache. The IMAP head
// (chassis/server/personality/imap) and the calendar head
// (chassis/server/personality/calendar) authenticate the same way, so one
// password minted for a persona can open both — the product decides whether
// the credential is shared by passing the same value to both account ops.
package apppass
...
// Argon2id parameters. Sized for the mail/calendar client login pattern — a
// desktop client opens 5–10 connections per wake, each with its own LOGIN,
// and a CalDAV client sends Basic credentials on every request — so the
// cost is kept at "tens of milliseconds, 16 MiB" per verify rather than the
// interactive-web-login maximum; the LoginCache absorbs the fan-out on top
// of this.
const (
	argonTime    uint32 = 3
...
var (
	...
	ErrBadHash = errors.New("apppass: malformed password hash")
...
// storage in an accounts table (imap_accounts.pw_hash, calendar_accounts.pw_hash).
func HashPassword(password string) (string, error) {
	salt := make([]byte, argonSaltLen)
	if _, err := rand.Read(salt); err != nil {
		return "", fmt.Errorf("apppass: salt: %w", err)
	}
...
func VerifyDummy(password string) bool {
	dummyOnce.Do(func() {
		h, err := HashPassword("txco-apppass-dummy-" + generateSecret(8))
		if err == nil {
			dummyHash = h
		}
	})
...
// the account ops. It is returned to the caller exactly once and only its
// hash is stored.
...
func generateSecret(n int) string {
	...
	for len(out) < n {
		if _, err := rand.Read(buf); err != nil {
			panic("apppass: crypto/rand unavailable: " + err.Error())
		}
		for _, c := range buf {
			if int(c) >= limit {
				continue
