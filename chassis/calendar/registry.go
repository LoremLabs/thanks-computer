...
type Config struct {
	// DBPath is the bundled SQLite backend's file path (--calendar-db-path).
	DBPath string
}
...
// what is available (so a misconfigured --calendar-store fails loudly).
func Open(name string, cfg Config) (*Store, error) {
	...
	if !ok {
		...
		return nil, fmt.Errorf("calendar: unknown store %q (available: %v)", name, avail)
	}
	return c(cfg)
}
