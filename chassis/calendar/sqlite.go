...
func init() {
	// Register the bundled "sqlite" backend (the default --calendar-store): a
	...
	// calendar index is written on every client PUT.
	Register("sqlite", func(cfg Config) (*Store, error) {
		dbPath := cfg.DBPath
		if dbPath == "" {
			dbPath = "./chassis/data/calendar.db"
		}
		if dir := filepath.Dir(dbPath); dir != "" && dir != "." {
			if err := os.MkdirAll(dir, 0o755); err != nil {
				return nil, fmt.Errorf("calendar: create dir %s: %w", dir, err)
			}
		}
