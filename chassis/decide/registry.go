...
// (see decide/vercel). Same pattern as chat.Register and embed.Register.
var (
...
func Open(name string, cfg Config) (Backend, error) {
	...
	registryMu.RUnlock()
	if !ok {
		return nil, fmt.Errorf("decide: unknown backend %q (available: %v)", name, Registered())
	}
	return c(cfg)
}

// Resolve picks a backend for one ai://decide dispatch, mirroring
// embed.Resolve:
//
...
func Resolve(providerHint string, cfg Config) (Backend, string, error) {
	...
	b, err := registry[registrationOrder[0]](cfg)
	if err != nil {
		return nil, "", err
	}
