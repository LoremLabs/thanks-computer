...
// "CALENDARS/<user>/<rel>", "CONTACTS/<user>/<rel>", "BLOBS/<rel>") plus the BLOBS/ bytes that must be streamed to the CAS
...
func collectStorePacks(stackDir string) ([]client.StackFile, []casUpload, error) {
	var out []client.StackFile
	for _, top := range []string{storeseed.DirVectors, storeseed.DirKV, storeseed.DirCalendars, storeseed.DirContacts} {
