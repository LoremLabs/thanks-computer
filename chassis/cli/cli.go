...
func Dispatch(args []string, stdout, stderr io.Writer) (status int, ok bool) {
	...
	case "notebook", "notebooks":
		// Read-only view of the notebooks a stack writes (`txco notebook
		// tail www task/42`). Same signed-target resolution as kv.
		return auth.RunNotebook(rest, stdout, stderr), true
	case "source", "sources":
...
func printUsage(w io.Writer) {
	...
	groups := []group{
		...
		{"Author & deploy", []row{
			...
			{"apply [<dir>]", muted("Deploy the OPS/ tree — code (operations + FILES); data is opt-in via ") + hint("`txco data`")},
