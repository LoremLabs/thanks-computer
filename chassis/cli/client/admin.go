...
type HTTPError struct {
	...
	Raw        string
	// Server and CFRay are captured from the response headers so a 5xx can be
	// attributed to the layer that produced it: a Cloudflare edge 502/504 (the
	// worker may still be running, just slow — the apply usually lands anyway)
	// vs a real worker 5xx like `database is locked`. Error() surfaces them on
	// every 5xx so the failure line is self-describing without TXCO_VERBOSE.
	Server string
	CFRay  string
}

// edge reports whether this response was generated by the Cloudflare edge
// (cf-ray header present, or Server: cloudflare) rather than the origin worker.
func (e *HTTPError) edge() bool {
	return e.CFRay != "" || strings.Contains(strings.ToLower(e.Server), "cloudflare")
}

// originTag annotates a 5xx with where it came from — the single most useful
// fact when an `apply` fails: did the edge time out in front of a working
// worker, or did the worker itself error? Empty for <500 (the tag would just
// be noise on a 4xx the caller already understands).
func (e *HTTPError) originTag() string {
	if e.StatusCode < 500 {
		return ""
	}
	where := "origin=worker"
	if e.edge() {
		where = "origin=cloudflare-edge"
	}
	tag := " [" + where
	if e.CFRay != "" {
		tag += " cf-ray=" + e.CFRay
...
func (e *HTTPError) Error() string {
	var base string
	if e.Code == "" {
		base = fmt.Sprintf("%s: %s", e.Status, e.Raw)
	} else {
		...
		if len(e.Detail) > 0 {
			base = fmt.Sprintf("%s: %s (detail=%v)", e.Status, msg, e.Detail)
		} else {
			base = fmt.Sprintf("%s: %s", e.Status, msg)
		}
	}
	return base + e.originTag()
}

// IsRetryable reports whether err is a transient failure worth retrying with
// backoff: a 5xx or 429 response (gateway timeouts, `database is locked`,
// overload), or a transport-layer network error (dial refused, TLS, timeout).
// Fatal 4xx (validation, auth, not-found) and a caller-canceled context are
// NOT retryable.
func IsRetryable(err error) bool {
	...
	if errors.As(err, &he) {
		return he.StatusCode == http.StatusTooManyRequests ||
			(he.StatusCode >= 500 && he.StatusCode <= 599)
	}
	// Not an HTTP-status error → it came back from the transport layer
	// (prettifyNetworkError wraps dial/TLS/timeout). Worth a retry.
	return true
}
