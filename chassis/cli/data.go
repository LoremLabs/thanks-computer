...
Deploy + inspect declarative store-seed packs (VECTORS/, KV/, BLOBS/, CALENDARS/, CONTACTS/). Data is opt-in:
`+"`txco apply`"+` deploys code only; data moves through these verbs.

Subcommands:
  apply [<dir>]            Deploy the local VECTORS/+KV/+BLOBS/+CALENDARS/+CONTACTS/ packs 
...
Deploy the local VECTORS/+KV/+BLOBS/+CALENDARS/+CONTACTS/ store-seed packs for every stack under
...
`)
	...
	if *f.jsonOut {
		...
		fmt.Fprintln(stdout, "no data packs found (VECTORS/, KV/, BLOBS/, CALENDARS/ or CONTACTS/) under any stack")
	}
	return rc
}
