...
// The bundled "sqlite" backend uses DBPath (same posture as
// notebook.Config).
type Config struct {
	// DBPath is the bundled SQLite backend's file path (--state-db-path).
	DBPath string
}
...
// Postgres backend the same way, so a transition on one node is
// presented by whichever node claims it.
...
// Open constructs the named backend. An unknown name is a startup error
// listing what is available (so a misconfigured --state-store fails
// loudly).
func Open(name string, cfg Config) (*Store, error) {
	...
	if !ok {
		...
		return nil, fmt.Errorf("state: unknown store %q (available: %v)", name, avail)
	}
	return c(cfg)
}
