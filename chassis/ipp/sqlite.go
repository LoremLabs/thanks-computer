...
// chassis runtime/scheduled/drive convention (WAL + busy_timeout, no
// cache=shared).
const sqliteBusyTimeoutMs = 15000

func init() {
	// Register the bundled "sqlite" backend (the default --ipp-store): a
	// SQLite file of its own. Never the runtime DB — the dbcache watcher
	// reloads the whole runtime mirror on any runtime-file write, and a job
	// row is written several times per print.
	Register("sqlite", func(cfg Config) (*Store, error) {
		dbPath := cfg.DBPath
		if dbPath == "" {
			dbPath = "./chassis/data/ipp.db"
		}
		if dir := filepath.Dir(dbPath); dir != "" && dir != "." {
			if err := os.MkdirAll(dir, 0o755); err != nil {
				return nil, fmt.Errorf("ipp: create dir %s: %w", dir, err)
			}
		}
		dsn := fmt.Sprintf("file:%s?mode=rwc&_journal_mode=WAL&_busy_timeout=%d&_txlock=immediate", dbPath, sqliteBusyTimeoutMs)
		db, err := sql.Open("sqlite3", dsn)
		if err != nil {
			return nil, fmt.Errorf("ipp: open %s: %w", dbPath, err)
		}
