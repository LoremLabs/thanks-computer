	...
	rows, err := c.pu.RuntimeDB.QueryContext(ctx, c.rb(`
		SELECT sf.path,
		 ...
		 WHERE s.tenant_id = ? AND s.name = ?
		   AND sf.path NOT LIKE 'VECTORS/%' AND sf.path NOT LIKE 'KV/%' AND sf.path NOT LIKE 'BLOBS/%' AND sf.path NOT LIKE 'CALENDARS/%' AND sf.path NOT LIKE 'CONTACTS/%'`),
		tenantID, name)
	if err != nil {
		return "", err
	}
...
func validateStackFilePath(p string) error {
	...
	// VECTORS/**, KV/**, CALENDARS/** and CONTACTS/** are declarative store-seed
	// packs (NDJSON), reconciled into the vector / KV / calendar / contacts stores on
	...
	// "CALENDARS/<username>/<calendar>.jsonl" and
	// "CONTACTS/<username>/<addressbook>.jsonl" (two) — so what each pack
	// owns is unambiguous.
	if storeseed.KindForPath(p) != "" {
		name := storeseed.PackName(p)
		if name == "" {
			return fmt.Errorf("store-seed packs must be a %q file: %s/<collection>, %s/<namespace>, %s/<username>/<calendar> or %s/<username>/<addressbook> (got %q)",
				storeseed.PackExt, storeseed.DirVectors, storeseed.DirKV, storeseed.DirCalendars, storeseed.DirContacts, p)
		}
...
func (c *Controller) handlePutDraftFiles(w http.ResponseWriter, r *http.Request) {
	...
	case "code":
		delQuery = `DELETE FROM stack_files WHERE version_id = ?
			   AND NOT (path LIKE 'VECTORS/%' OR path LIKE 'KV/%' OR path LIKE 'BLOBS/%' OR path LIKE 'CALENDARS/%' OR path LIKE 'CONTACTS/%')`
	case "data":
		delQuery = `DELETE FROM stack_files WHERE version_id = ?
			   AND (path LIKE 'VECTORS/%' OR path LIKE 'KV/%' OR path LIKE 'BLOBS/%' OR path LIKE 'CALENDARS/%' OR path LIKE 'CONTACTS/%')`
	}
