...
type Config struct {
	...
	Personalities                string   `id:"personalities" default:"cron,tcp,web,admin" desc:"Head types to start. Comma delimited. {cron,tcp,web,admin,lmtp,sweep,dns,mailmap,scheduled,imap,websocket,calendar,contacts,webdav,source,ipp} (cron,tcp,web,admin)"`
	...
	IMAPLoginRate                int      `id:"imap-login-rate" default:"10" desc:"Max LOGIN attempts per minute, applied per client IP and per username independently; attempts over the limit answer NO [LIMIT] without touching the password. 0 disables. (10)"`
	...
	CalendarLoginRate            int      `id:"calendar-login-rate" default:"30" desc:"Max Basic-auth verifications per minute, applied per client IP and per username independently, counted only on verified-login-cache misses (a CalDAV client re-authenticates every request; hits cost nothing). Over the limit answers 429 without touching the password. Behind a front proxy the client IP is the proxy's, so the per-IP bucket is shared by every client it fronts. 0 disables. (30)"`
	...
	ContactsLoginRate            int      `id:"contacts-login-rate" default:"30" desc:"Max Basic-auth verifications per minute, applied per client IP and per username independently, counted only on verified-login-cache misses (a CardDAV client re-authenticates every request; hits cost nothing). Over the limit answers 429 without touching the password. Behind a front proxy the client IP is the proxy's, so the per-IP bucket is shared by every client it fronts. 0 disables. (30)"`
	...
	DriveLoginRate               int      `id:"drive-login-rate" default:"30" desc:"Max Basic-auth verifications per minute, applied per client IP and per username independently, counted only on verified-login-cache misses (a WebDAV client re-authenticates every request; hits cost nothing). Over the limit answers 429 without touching the password. Behind a front proxy the client IP is the proxy's, so the per-IP bucket is shared by every client it fronts. 0 disables. (30)"`
	...
	IPPAuthRate                  int      `id:"ipp-auth-rate" default:"30" desc:"Max Basic-auth verifications per minute, applied per client IP and per tenant independently, counted only on verified-login-cache misses (a print client re-authenticates every request; hits cost nothing, guesses always miss). Over it the answer is 429 — for a correct guess too. 0 disables. (30)"`
