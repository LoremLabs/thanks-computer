...
type Config struct {
	...
	LoginRate                    int      `id:"login-rate" default:"30" desc:"Max password checks per minute, per client IP and per principal, shared by every head that signs in with a credential (imap, calendar, contacts, webdav): a guess over CalDAV spends the same budget as one over IMAP. Counted only on verified-login-cache misses (a DAV client sends its password with every request; hits cost nothing). Over the limit answers 429 (DAV) or NO [LIMIT] (IMAP) without touching the password. Behind a front proxy the DAV heads see the proxy's address, so every client it fronts shares one per-IP bucket, unless --web-trusted-proxies names the proxy. 0 disables. (30)"`
