...
type Config struct {
	...
	AdminAllowOpen               bool     `id:"admin-allow-open" default:"false" desc:"Allow the admin API to run WITHOUT authentication (grants admin:all to any caller) when no --admin-user/--admin-pass are set and --auth-mode is basic/both. Off by default → fail closed. Implied true when --env starts with 'dev' (local convenience). NEVER enable in a network-reachable production environment. (false)"`
	...
	DbAuthDsn                    string   `id:"db-auth-dsn" default:"file:$db-root-dir/auth-$env.db" desc:"DSN for auth database. Two planes share it: the account plane (actors, keys, memberships, invitations, browser sessions), used only when the admin personality is active; and the stack-plane identity tables (users, principal bindings, credentials — txco://user/*, txco://credential/*), opened on every node. A node without the admin personality that cannot open it keeps serving and those ops answer txco_user_disabled. Default is a local SQLite file (file:./chassis/data/db/auth-$env.db);"`
	...
	StateStore                   string   `id:"state-store" default:"sqlite" desc:"Backend for the state store (txco://state/*: durable state records, and the transition outbox the 'state' personality presents into each tenant's _state stack): sqlite (bundled), or a backend an overlay registers (the hosted build: postgres, reading TXCO_DB_STATE_DSN, else TXCO_DB_AUTH_DSN). sqlite is a per-node file at --state-db-path and opens only with the 'state' personality; a shared backend opens on every node. (sqlite)"`
	StateDBPath                  string   `id:"state-db-path" default:"./chassis/data/state.db" desc:"Path to the bundled state SQLite file (used by --state-store=sqlite; ignored by other backends). Its parent dir is created on boot. (./chassis/data/state.db)"`
	StatePeriod                  int      `id:"state-period" default:"5" desc:"Seconds between state-event poll passes (the 'state' personality). A node that commits a transition presents it at once; the period is the worst case for an event another node committed. (5)"`
	StateMaxInflight             int      `id:"state-max-inflight" default:"32" desc:"Max concurrent state-event presentations per poll pass. (32)"`
	StateAcceptTimeout           int      `id:"state-accept-timeout" default:"30" desc:"Seconds the state dispatcher waits for the chassis to accept a presented event (routed into the tenant and admitted) before returning it to the queue with an attempt counted; an event accepted late is presented twice. (30)"`
	StateRunTimeout              int      `id:"state-run-timeout" default:"600" desc:"Seconds a _state run may take once accepted. The dispatcher does not wait for it; this bounds the run itself. (600)"`
	StateStaleAfter              int      `id:"state-stale-after" default:"300" desc:"Seconds a claimed state event may sit before the reclaim sweep returns it to pending so another node presents it — crash recovery for a node that died mid-presentation. Must exceed --state-accept-timeout. (300)"`
	StateMaxAttempts             int      `id:"state-max-attempts" default:"5" desc:"Presentations of one state event that may time out or be reclaimed before it is marked dead (the crash-loop guard); admission denials do not count. (5)"`
	StateRetention               int      `id:"state-retention" default:"2592000" desc:"Seconds to keep terminal (done/skipped/dead) state-event rows before the dispatcher purges them; pending rows are never purged. (2592000 = 30d)"`
	StateMaxDataBytes            int      `id:"state-max-data-bytes" default:"65536" desc:"Byte cap for one state record's data (txco://state/create and /transition). 0 = unlimited. (65536)"`
	SourcePeriod                 int      `id:"source-period" default:"30" desc:"Seconds between source-watcher poll passes (the 'source' personality). Each pass reclaims stale claims, claims due sources (next_poll_at <= now), polls each for new items, fires them into the source's <stack>/_source/0, then reschedules the source its own interval out (30)."`
	SourceMaxInflight            int      `id:"source-max-inflight" default:"8" desc:"Max concurrent source connections a poll pass opens; bounds fan-out (and outbound sockets) when many sources come due at once (8)."`
	SourceStaleAfter             int      `id:"source-stale-after" default:"600" desc:"Seconds a claimed source may sit before the reclaim sweep returns it to idle so another node retries — crash recovery for a node that died mid-poll (600)."`
	SourceBatch                  int      `id:"source-batch" default:"50" desc:"Max items one source yields per poll pass, so a first sync of a large mailbox drains over several passes instead of one huge fetch (50)."`
	SourceDispatchTimeout        int      `id:"source-dispatch-timeout" default:"60" desc:"Seconds to wait for one fetched item's run to complete before treating it as failed (the item is not acked/advanced and is retried next pass) (60)."`
	...
	KVStoreAddrs                 []string `id:"kvstore-addrs" default:"./chassis/data/kv" desc:"KVStore address(es). For redis: a bare host:port (plaintext), or a redis://|rediss:// URL — the rediss scheme selects TLS and the URL's userinfo carries the username/password (e.g. a managed Redis: rediss://default:<token>@<db>.upstash.io:6379). For boltdb: the on-disk file path. (./chassis/data/kv)"`
	KVStoreBucket                string   `id:"kvstore-bucket" default:"txco" desc:"KVStore bucket (boltdb only) (txco)"`
	KVStorePassword              string   `id:"kvstore-password" default:"" desc:"Password for the redis KV backend. Empty (default) sends no AUTH — correct for boltdb or an unauthenticated redis on a trusted private network. Set it to match the redis 'requirepass' to authenticate the redis KV. A password in a rediss://user:pass@… kvstore-addrs URL takes precedence over this. ()"`
	...
	BlobMaxBytes                 int      `id:"blob-max-bytes" default:"33554432" desc:"Decoded byte cap for a txco://blob/put value and a blob/get read into the envelope; larger errors as blob.error too_large. 0 = unlimited. BLOBS/ seed packs stream over the blob plane and are bounded by --dataset-max-file-bytes instead (33554432, 32MiB)."`
	...
	MaxFuelPerRequest            int      `id:"max-fuel-per-request" default:"100000" desc:"Per-request fuel ceiling. Each accounted action (scope-enter=10, repeat-transition=50, EXEC=25, secret-materialize=100, nano-op compute=10/ms, workspace wall-clock=10/s) accrues weighted cost; exhaustion halts the request with a structured txco_fuel_exhausted error. Catches expensive non-loop work (wide fan-out, secret floods). Set to 0 to disable enforcement (metering continues). (100000)"`
	...
	OpLoopMax                    int      `id:"op-loop-max" default:"1000" desc:"Hard cap on LOOP … MAX, the per-op pass ceiling for a LOOP clause. An op asking for more is rejected at dispatch and dropped from the merge for that request. 0 = no cap. (1000)"`
	LoopTimeout                  string   `id:"loop-timeout" default:"60s" desc:"Default per-op timeout for a rule with a LOOP clause when WITH timeout is absent; it bounds the whole loop, every pass and pause included. Replaces the chassis-wide op-timeout (default 5s), which is too tight for a poll. Still capped by op-timeout-max. (60s)"`
	...
	DecideZeroDataRetention      bool     `id:"decide-zero-data-retention" default:"true" desc:"Ask the ai://decide provider not to retain request data. (true)"`
	...
	SearchStore                  string   `id:"search-store" default:"bleve" desc:"Backend for the txco://search/* lexical search ops: {bleve,none}. bleve is the bundled Bleve engine, one index per collection under --search-path. none turns lexical search off, and the ops answer txco_search_disabled. (bleve)"`
	SearchPath                   string   `id:"search-path" default:"./chassis/data/search" desc:"Root directory of the bundled lexical search store. Created on boot. Each (tenant, collection) is its own index directory beneath it, named by hash. (./chassis/data/search)"`
	SearchMaxOpenIndexes         int      `id:"search-max-open-indexes" default:"64" desc:"How many collection indexes the bundled lexical search store holds open at once. Beyond it the least recently used idle index is closed and reopens on its next use. (64)"`
	...
	TCPMaxConns                  int      `id:"tcp-max-conns" default:"0" desc:"Max open TCP-head connections on this node, all listeners; past the cap a new connection is answered with one '503 too many connections' line and closed. 0 disables the cap. (0)"`
	TCPDrainTimeout              string   `id:"tcp-drain-timeout" default:"5s" desc:"On shutdown every open TCP-head connection is closed; this bounds how long the chassis waits for their handlers to unwind. (5s)"`
	TCPMaxConnsPerTenant         int      `id:"tcp-max-conns-per-tenant" default:"0" desc:"Max open TCP-head connections per tenant on this node, counted once the connect run has routed; past it the connection is answered with one '503 too many connections' line and closed. 0 disables the cap. (0)"`
	TCPConnFuel                  int      `id:"tcp-conn-fuel" default:"100" desc:"Fuel charged for accepting one TCP-head connection, pre-stamped on its connect run so it lands on that run's usage event (the routed tenant's; _sys for an unrouted connection). Bytes a connection moves outside a run are charged separately at the blob/KV rate (100 per MiB) on its next event. 0 disables the connect charge. (100)"`
	TCPHandshakeTimeout          string   `id:"tcp-handshake-timeout" default:"5s" desc:"Bound on the TLS handshake for a ';tls' listener in --tcp-listen-addrs (a client that stalls its ClientHello is dropped), and on the PROXY header for a ';proxy=' one. (5s)"`
	DNSListenAddrs               []string `id:"dns-listen-addrs" default:":5354" desc:"Authoritative-DNS listen addresses. Comma list; a bare ':port' or 'host:port' binds UDP AND TCP on that address; a 'udp:host:port' or 'tcp:host:port' entry binds that ONE transport (for a front that delivers the two on different addresses — Fly needs 'udp:fly-global-services:53,tcp:0.0.0.0:53'). DNS requires both transports to reach the head one way or another; a one-transport bind logs a warning. NOTE: the 'udp:'/'tcp:' prefixes used to be ignored (both transports bound); they are now honoured. Default :5354 — deliberately NOT :5353 (that's mDNS on macOS and clashes); the well-known DNS port 53 needs root/CAP_NET_BIND_SERVICE or a front LB. Set empty to disable the head even when 'dns' is in --personalities. (:5354)"`
	DNSChallengeStore            string   `id:"dns-challenge-store" default:"" desc:"Where the transient _acme-challenge TXT records of an ACME DNS-01 solve are kept while a certificate issues: '' or 'memory' (default) = in this process, correct when it is the ONLY authoritative nameserver; 'kv' = the shared --kvstore (requires --kvstore=redis; refused otherwise), so a challenge written on any head — by the bundled solver or the RFC2136 receiver — is served by every head, which a CA validating against a multi-nameserver NS set requires. Makes heads SERVE one challenge; it does not coordinate certificate issuance between them (that is --cert-storage-dsn). ()"`
	...
	DNSIMAPSPort                 int      `id:"dns-imaps-port" default:"0" desc:"Port advertised in synthesized RFC 6186 _imaps._tcp SRV records for delegated zones — at the apex and every per-stack host, target = the name itself; the default-suffix wildcard zone advertises imap.<suffix> (one target under the wildcard certificate) — for mail clients that implement RFC 6186 discovery (Thunderbird does not — it fetches the /.well-known/autoconfig XML a stack can serve; see docs/advanced/protocols/imap.md). 0 (default) emits no SRV. Set 993 only when an IMAPS front door serves every hostname of the zone. (0)"`
	DNSCalDAVSPort               int      `id:"dns-caldavs-port" default:"0" desc:"Port advertised in synthesized RFC 6764 _caldavs._tcp SRV records for delegated zones (at the apex and every per-stack host, target = the name itself, with a TXT \"path=/.well-known/caldav\" beside it; the default-suffix wildcard zone advertises caldav.<suffix>) — for calendar clients that discover a server from an e-mail address. 0 (default) emits no SRV. Set 443 when the 'calendar' personality serves every hostname of the zone. (0)"`
	DNSCardDAVSPort              int      `id:"dns-carddavs-port" default:"0" desc:"Port advertised in synthesized RFC 6764 _carddavs._tcp SRV records for delegated zones (at the apex and every per-stack host, target = the name itself, with a TXT \"path=/.well-known/carddav\" beside it; never on the default-suffix wildcard) — for contacts apps that discover a server from an e-mail address. 0 (default) emits no SRV. Set 443 when the 'contacts' personality serves every hostname of the zone. (0)"`
	DNSIPP                       bool     `id:"dns-ipp" default:"false" desc:"Synthesize ipp.<zone> A/AAAA (the edge IPs) for every delegated pattern zone — the hostname the 'ipp' personality answers on (ipps://ipp.<zone>/p/<printer>). Computed at snapshot build, so zones created before the flag was set get the name on the next reload. false (default) emits nothing. Set true when the 'ipp' personality serves the fleet. (false)"`
	...
	DNSObserveSample             int      `id:"dns-observe-sample" default:"1" desc:"DNS observe tap: after a query in a delegated zone is answered on the wire, a compact record of it (question, client, reply) is dispatched fire-and-forget into the zone tenant's _dns stack — the stack's existence is the subscription, like _cron. 1 (default) taps every answered query; N taps one in N; 0 disables the tap. Queries the head refuses (unserved names), drops (RRL), or receives as RFC2136 UPDATEs are never tapped. Each tapped query is a normal (fuel-metered) run of the tenant's _dns stack. (1)"`
	DNSObserveMaxInflight        int      `id:"dns-observe-max-inflight" default:"8" desc:"Max concurrent observe-tap dispatches into _dns stacks. Tapped queries wait in a bounded in-memory queue; when it is full the tap drops (chassis.dns.observe outcome=dropped) rather than ever delaying a wire reply. (8)"`
	DNSStackDeadlineMs           int      `id:"dns-stack-deadline-ms" default:"1500" desc:"Stack-answered zones (txco dns zone set <origin> --answer stack): how long the head waits for the tenant's _dns stack to EMIT @dns.res before answering with the zone's fallback (proposal or servfail). The run is not cancelled at the deadline — a late answer still warms the answer cache. Resolvers retry at ~1-2s, so keep this well under that. (1500)"`
	DNSStackDispatchPerSec       int      `id:"dns-stack-dispatch-per-sec" default:"20" desc:"Per-zone ceiling on _dns stack dispatches per second for stack-answered zones; queries over the limit answer with the zone's fallback instead of queueing (a random-subdomain flood defeats the answer cache by construction, so this is the real cost ceiling). 0 disables the limiter (dev). (20)"`
	IMAPListenAddrs              []string `id:"imap-listen-addrs" default:"" desc:"Plaintext IMAP listen addresses for the 'imap' personality. Comma list of ':port' or 'host:port'. Empty (default) — no default port: a front proxy (edge Caddy layer4, nginx stream) terminates TLS and forwards here, or use --imap-tls-addrs. The head stays silent when both lists are empty even with 'imap' in --personalities. ()"`
	IMAPTLSAddrs                 []string `id:"imap-tls-addrs" default:"" desc:"Implicit-TLS (IMAPS) listen addresses, e.g. ':993'. The certificate comes from --imap-tls-cert-file/--imap-tls-key-file when set, else from the bundled cert manager (ACME DNS-01 against the 'dns' head; --imap-hostname is added to the managed set). Empty (default) disables IMAPS. ()"`
	IMAPHostname                 string   `id:"imap-hostname" default:"" desc:"Public hostname of the IMAP head (e.g. imap.example.com). Added to the bundled cert manager's managed names when --imap-tls-addrs is set without cert files. ()"`
	IMAPTLSCertFile              string   `id:"imap-tls-cert-file" default:"" desc:"PEM certificate chain for --imap-tls-addrs, obtained by anything else (certbot, a same-host Caddy). With --imap-tls-key-file it bypasses the bundled cert manager. ()"`
	IMAPTLSKeyFile               string   `id:"imap-tls-key-file" default:"" desc:"PEM private key matching --imap-tls-cert-file. ()"`
	IMAPSelfSigned               bool     `id:"imap-self-signed" default:"false" desc:"Serve a self-signed certificate (minted in memory at boot; loopback + the dev-local hostname patterns + --imap-hostname) when no cert files and no bundled cert manager apply — so a desktop mail client, which refuses LOGIN over plaintext, can connect with STARTTLS on --imap-listen-addrs or IMAPS on --imap-tls-addrs and trust the certificate once. txco dev --imap turns this on. Never for a public deployment. (false)"`
	IMAPWireDebug                bool     `id:"imap-wire-debug" default:"false" desc:"Log every IMAP command and response line at DEBUG (chassis.imap.wire). Includes credentials in the clear — for diagnosing a mail client on a dev machine only. (false)"`
	IMAPStore                    string   `id:"imap-store" default:"sqlite" desc:"Backend for the IMAP mailbox index (accounts, mailboxes, message rows; bytes live in the content store): sqlite (bundled), or a backend an overlay registers (the hosted build: postgres, reading TXCO_DB_AUTH_DSN). sqlite is a per-node file at --imap-db-path, opened only when 'imap' is in --personalities — never the runtime DB. Any other backend is SHARED and is opened on every node, so txco://imap/* on any node project into the one index the head serves; if it fails to open on a node without the head, the ops answer txco_imap_disabled there until restart. (sqlite)"`
	IMAPDBPath                   string   `id:"imap-db-path" default:"./chassis/data/imap.db" desc:"Path to the bundled IMAP index SQLite file (used by --imap-store=sqlite; ignored by other backends). Its parent dir is created on boot. (./chassis/data/imap.db)"`
	NotebookStore                string   `id:"notebook-store" default:"sqlite" desc:"Backend for the notebook store (txco://notebook/*: append-only records per tenant/namespace/name): sqlite (bundled), or a backend an overlay registers (the hosted build: postgres, reading TXCO_DB_NOTEBOOK_DSN, else TXCO_DB_AUTH_DSN). sqlite is a per-node file at --notebook-db-path — never the runtime DB. The store opens on every node; a non-sqlite backend that fails to open answers txco_notebook_disabled until restart. (sqlite)"`
	NotebookDBPath               string   `id:"notebook-db-path" default:"./chassis/data/notebook.db" desc:"Path to the bundled notebook SQLite file (used by --notebook-store=sqlite; ignored by other backends). Its parent dir is created on boot. (./chassis/data/notebook.db)"`
	NotebookMaxEntryBytes        int      `id:"notebook-max-entry-bytes" default:"65536" desc:"Per-entry byte cap for the data of a txco://notebook/append; a larger value errors as txco_notebook_too_large. 0 = unlimited. (65536, 64KiB)"`
	NotebookMaxReadRows          int      `id:"notebook-max-read-rows" default:"1000" desc:"Page ceiling for txco://notebook/read and the admin read endpoint; a WITH limit clamps under it. The default page (no limit given) is 100. (1000)"`
	NotebookMaxTTL               int      `id:"notebook-max-ttl" default:"0" desc:"Upper bound in seconds for a notebook entry TTL (WITH ttl, or a head default); longer requests are clamped. 0 = unlimited. (0)"`
	NotebookMaxExportBytes       int      `id:"notebook-max-export-bytes" default:"8388608" desc:"Byte cap for one txco://notebook/export response body; the export stops on a line boundary, reports truncated, and returns a cursor to resume from. 0 = unlimited. (8388608, 8MiB)"`
	NotebookSweepPeriod          string   `id:"notebook-sweep-period" default:"10m" desc:"How often each node deletes expired notebook entries, in batches of --notebook-sweep-batch. Expired entries are already invisible to reads; the sweep reclaims space. 0 disables. (10m)"`
	NotebookSweepBatch           int      `id:"notebook-sweep-batch" default:"1000" desc:"Rows deleted per notebook sweep statement; a sweep repeats until a batch comes back short. (1000)"`
	IMAPInsecureAuth             bool     `id:"imap-insecure-auth" default:"false" desc:"Accept LOGIN on a plaintext connection. For txco dev (loopback) and behind-a-proxy deployments where TLS terminates upstream; leave false when clients reach the plaintext port directly. (false)"`
	IMAPMaxConnsPerAccount       int      `id:"imap-max-conns-per-account" default:"16" desc:"Max simultaneous authenticated IMAP connections per account (desktop clients open 5-10 per wake). 0 disables the cap. (16)"`
	IMAPLoginRate                int      `id:"imap-login-rate" default:"10" desc:"Max IMAP LOGIN commands per minute, applied per client IP and per username independently, before any lookup: a flood guard for the command itself; attempts over the limit answer NO [LIMIT]. Password checks are limited separately, across every head, by --login-rate. 0 disables. (10)"`
	IMAPRefuseBareUsernames      bool     `id:"imap-refuse-bare-usernames" default:"false" desc:"Refuse an IMAP LOGIN whose username has no @. By default a bare local part (\"paris\") signs in when it names exactly one account on the whole chassis — mail clients send it when the address domain and the server name line up — but that lookup cannot be scoped to a tenant (the IMAP head knows no hostname at login). Each such login is logged with bare=true: read that before turning this on, because every client that relies on it must then be given its full address. A later release turns it on by default. (false)"`
	LoginRate                    int      `id:"login-rate" default:"30" desc:"Max password checks per minute, per client IP and per principal, shared by every head that signs in with a credential (imap, calendar, contacts, webdav, ipp): a guess over CalDAV spends the same budget as one over IMAP. Counted only on verified-login-cache misses (a DAV client sends its password with every request; hits cost nothing). Over the limit answers 429 (DAV, IPP) or NO [LIMIT] (IMAP) without touching the password. Behind a front proxy the DAV and IPP heads see the proxy's address, so every client it fronts shares one per-IP bucket, unless --web-trusted-proxies names the proxy. 0 disables. (30)"`
	IMAPSyncInterval             string   `id:"imap-sync-interval" default:"15s" desc:"How often the IMAP head re-reads the modseq of every SELECTed mailbox from the index and delivers what changed (EXISTS / EXPUNGE / FETCH FLAGS, including to IDLE clients). Writes made in this process are delivered at once; the tick covers writes from other nodes against a shared index (ops elsewhere, a second head). One indexed point read per selected mailbox per tick, a full re-read only when the modseq moved. 0 disables the tick: remote changes then surface on a client's next command only. (15s)"`
	IMAPRespTimeout              string   `id:"imap-resp-timeout" default:"30s" desc:"Answer lane: how long the IMAP head waits for the tenant's _imap stack to EMIT @imap.res before answering NO [UNAVAILABLE] for a mailbox whose policy says 'stack'. The run keeps its own 60s context; a late answer is discarded. (30s)"`
	IMAPObserveSample            int      `id:"imap-observe-sample" default:"1" desc:"Observe lane: after a client mutation commits (append, move, copy, expunge, create, delete, rename; flags only where a mailbox opts in), a record of it is dispatched fire-and-forget into the account tenant's _imap stack — the stack's existence is the subscription, like _dns. 1 (default) observes every mutation; N one in N; 0 disables the lane. Each observed mutation is a normal (fuel-metered) run. (1)"`
	IMAPObserveMaxInflight       int      `id:"imap-observe-max-inflight" default:"8" desc:"Max concurrent observe-lane dispatches into _imap stacks; over the bounded queue the observation is dropped (chassis.imap.observe outcome=dropped) rather than ever delaying the client. (8)"`
	IMAPProxyProtocol            []string `id:"imap-proxy-protocol" default:"" desc:"CIDRs of front proxies allowed to prefix connections with a PROXY protocol (v1/v2) header — the real client IP then survives a TLS-terminating edge (Caddy layer4, haproxy) for throttles and the envelope's _txc.client.ip. Connections from other addresses are served as-is (a header from them is not honoured). Empty (default) disables. ()"`
	WebTrustedProxies            []string `id:"web-trusted-proxies" default:"" desc:"CIDRs (or bare IPs) of the HTTP proxies in front of the web head. When a request's socket peer is one of them, the client address is read from X-Forwarded-For: the first address from the right that is not itself a trusted proxy, so nothing a client types into the header is ever reached. It is what the calendar, contacts, webdav and ipp heads key their per-IP login limits on (and log), what the websocket head records, and the ip in the web access log. From any other peer the header is ignored. Empty (the default) trusts nobody: the client is the socket peer, which behind a proxy is the proxy — every client then shares one per-IP login budget. Several entries may be separated by spaces or commas. The HTTP counterpart of --imap-proxy-protocol. (none)"`
	IMAPAppendMaxBytes           int      `id:"imap-append-max-bytes" default:"33554432" desc:"Max rendered message size a txco://imap/append may store. Bytes go to the content store and only references ride the envelope, so this matches --blob-max-bytes rather than an envelope cap. (33554432, 32 MiB)"`
	CalendarStore                string   `id:"calendar-store" default:"sqlite" desc:"Backend for the calendar index (accounts, calendars, iCalendar objects — the objects live here, never the blob CAS): sqlite (bundled), or a backend an overlay registers (the hosted build: postgres, reading TXCO_DB_AUTH_DSN). sqlite is a per-node file at --calendar-db-path, opened only when 'calendar' is in --personalities — never the runtime DB. Any other backend is SHARED and is opened on every node, so txco://calendar/* on any node project into the one index the head serves; if it fails to open on a node without the head, the ops answer txco_calendar_disabled there until restart. (sqlite)"`
	CalendarDBPath               string   `id:"calendar-db-path" default:"./chassis/data/calendar.db" desc:"Path to the bundled calendar index SQLite file (used by --calendar-store=sqlite; ignored by other backends). Its parent dir is created on boot. (./chassis/data/calendar.db)"`
	CalendarPathPrefix           string   `id:"calendar-path-prefix" default:"/dav" desc:"URL prefix the 'calendar' personality reserves on EVERY hostname the web head serves (plus /.well-known/caldav, which redirects into it): CalDAV under <prefix>/<username>/…, ICS feeds under <prefix>/feed/<token>.ics. Requests here are answered from the calendar index and never run a stack. Move it when a tenant's own routes need /dav. (/dav)"`
	CalendarInsecureAuth         bool     `id:"calendar-insecure-auth" default:"false" desc:"Accept Basic authentication on a plaintext request. Off (default), a request must arrive over TLS — the chassis's own --web-tls-addr listener, or X-Forwarded-Proto: https from a front proxy that terminates TLS (the same trust the admin surface extends). For txco dev only. (false)"`
	CalendarRespTimeout          string   `id:"calendar-resp-timeout" default:"30s" desc:"Answer lane: how long the calendar head waits for the tenant's _calendar stack to EMIT @calendar.res before answering 403 for a calendar whose policy says 'stack'. The run keeps its own 60s context; a late answer is discarded. (30s)"`
	CalendarObserveSample        int      `id:"calendar-observe-sample" default:"1" desc:"Observe lane: after a client mutation commits (put, delete, mkcalendar, remove, proppatch where the calendar's policy says observe), a record of it is dispatched fire-and-forget into the account tenant's _calendar stack — the stack's existence is the subscription, like _imap. 1 (default) observes every mutation; N one in N; 0 disables the lane. Each observed mutation is a normal (fuel-metered) run. (1)"`
	CalendarObserveMaxInflight   int      `id:"calendar-observe-max-inflight" default:"8" desc:"Max concurrent observe-lane dispatches into _calendar stacks; over the bounded queue the observation is dropped (chassis.calendar.observe outcome=dropped) rather than ever delaying the client. (8)"`
	CalendarObjectMaxBytes       int      `id:"calendar-object-max-bytes" default:"1048576" desc:"Size cap for one iCalendar object, for txco://calendar/put and a client PUT (413 over it; also advertised as CALDAV:max-resource-size). Objects live in the index, so keep this small. (1048576, 1 MiB)"`
	CalendarFeedMaxAge           int      `id:"calendar-feed-max-age" default:"300" desc:"Cache-Control max-age (seconds) on ICS feed responses, so a subscription poller (Google Calendar, Outlook) that respects it re-reads at most this often. The feed carries an ETag either way. (300)"`
	ContactsStore                string   `id:"contacts-store" default:"sqlite" desc:"Backend for the contacts index (accounts, address books, vCard objects — the objects live here, never the blob CAS): sqlite (bundled), or a backend an overlay registers (the hosted build: postgres, reading TXCO_DB_AUTH_DSN). sqlite is a per-node file at --contacts-db-path, opened only when 'contacts' is in --personalities — never the runtime DB. Any other backend is SHARED and is opened on every node, so txco://contacts/* on any node project into the one index the head serves; if it fails to open on a node without the head, the ops answer txco_contacts_disabled there until restart. (sqlite)"`
	ContactsDBPath               string   `id:"contacts-db-path" default:"./chassis/data/contacts.db" desc:"Path to the bundled contacts index SQLite file (used by --contacts-store=sqlite; ignored by other backends). Its parent dir is created on boot. (./chassis/data/contacts.db)"`
	ContactsPathPrefix           string   `id:"contacts-path-prefix" default:"/carddav" desc:"URL prefix the 'contacts' personality reserves on EVERY hostname the web head serves (plus /.well-known/carddav, which redirects into it): CardDAV under <prefix>/<username>/addressbooks/…. Requests here are answered from the contacts index and never run a stack. Must differ from --calendar-path-prefix. Move it when a tenant's own routes need /carddav. (/carddav)"`
	ContactsInsecureAuth         bool     `id:"contacts-insecure-auth" default:"false" desc:"Accept Basic authentication on a plaintext request. Off (default), a request must arrive over TLS — the chassis's own --web-tls-addr listener, or X-Forwarded-Proto: https from a front proxy that terminates TLS (the same trust the admin surface extends). For txco dev only. (false)"`
	ContactsRespTimeout          string   `id:"contacts-resp-timeout" default:"30s" desc:"Answer lane: how long the contacts head waits for the tenant's _contacts stack to EMIT @contacts.res before answering 403 for an address book whose policy says 'stack'. The run keeps its own 60s context; a late answer is discarded. (30s)"`
	ContactsObserveSample        int      `id:"contacts-observe-sample" default:"1" desc:"Observe lane: after a client mutation commits (put, delete, mkaddressbook, remove, proppatch where the address book's policy says observe), a record of it is dispatched fire-and-forget into the account tenant's _contacts stack — the stack's existence is the subscription, like _calendar. 1 (default) observes every mutation; N one in N; 0 disables the lane. Each observed mutation is a normal (fuel-metered) run. (1)"`
	ContactsObserveMaxInflight   int      `id:"contacts-observe-max-inflight" default:"8" desc:"Max concurrent observe-lane dispatches into _contacts stacks; over the bounded queue the observation is dropped (chassis.contacts.observe outcome=dropped) rather than ever delaying the client. (8)"`
	ContactsObjectMaxBytes       int      `id:"contacts-object-max-bytes" default:"1048576" desc:"Size cap for one vCard object, for txco://contacts/put and a client PUT (413 over it; also advertised as CARDDAV:max-resource-size). A contact photo is a few hundred KB; objects live in the index, so keep this small. (1048576, 1 MiB)"`
	DriveStore                   string   `id:"drive-store" default:"sqlite" desc:"Backend for the drive INDEX (collections, accounts, resource rows — names, versions, hierarchy; the bytes live in --drive-objects): sqlite (bundled), or a backend an overlay registers (the hosted build: postgres, reading TXCO_DB_AUTH_DSN). sqlite is a per-node file at --drive-db-path, opened only when 'webdav' is in --personalities — never the runtime DB. Any other backend is SHARED and is opened on every node, so txco://drive/* on any node project into the one index the head serves; if it fails to open on a node without the head, the ops answer txco_drive_disabled there until restart. (sqlite)"`
	DriveDBPath                  string   `id:"drive-db-path" default:"./chassis/data/drive.db" desc:"Path to the bundled drive index SQLite file (used by --drive-store=sqlite; ignored by other backends). Its parent dir is created on boot. (./chassis/data/drive.db)"`
	DriveObjects                 string   `id:"drive-objects" default:"file" desc:"Object store for drive resource BYTES: file (bundled, a directory at --drive-objects-file-dir), or a backend an overlay registers (the hosted build: s3). Every node that opens the drive index opens this too. (file)"`
	DriveObjectsFileDir          string   `id:"drive-objects-file-dir" default:"./chassis/data/drive" desc:"Root directory for the bundled file drive object store (used by --drive-objects=file). One immutable file per stored version under <tenant>/<collection>/<resource>/<version>. (./chassis/data/drive)"`
	DrivePathPrefix              string   `id:"drive-path-prefix" default:"/drive" desc:"URL prefix the 'webdav' personality reserves on EVERY hostname the web head serves: the mount URL is https://<host><prefix>/ and the authenticated account's collection is its root. Requests here are answered from the drive index and never run a stack. Must differ from --calendar-path-prefix and --contacts-path-prefix. Move it when a tenant's own routes need /drive. (/drive)"`
	DriveInsecureAuth            bool     `id:"drive-insecure-auth" default:"false" desc:"Accept Basic authentication on a plaintext request. Off (default), a request must arrive over TLS — the chassis's own --web-tls-addr listener, or X-Forwarded-Proto: https from a front proxy that terminates TLS (the same trust the admin surface extends). For txco dev only. (false)"`
	DriveMaxFileBytes            int      `id:"drive-max-file-bytes" default:"4294967296" desc:"Size cap for one drive file, for a client PUT (413 over it) and txco://drive/put. Bytes stream to the object store, so this bounds storage, not memory. 0 = unlimited. (4294967296, 4 GiB)"`
	DriveMaxCollectionBytes      int      `id:"drive-max-collection-bytes" default:"0" desc:"Cap on a collection's total live bytes; a write past it answers 507 (WebDAV) or txco_drive_quota (ops). 0 = unlimited. (0)"`
	DriveMaxResources            int      `id:"drive-max-resources" default:"0" desc:"Cap on a collection's live files + directories; a create past it answers 507 (WebDAV) or txco_drive_quota (ops). 0 = unlimited. (0)"`
	DriveOpMaxBytes              int      `id:"drive-op-max-bytes" default:"33554432" desc:"Cap on the bytes txco://drive/put accepts from the envelope and txco://drive/get returns into it (both are buffered JSON, unlike the streaming head). A larger file is stored and served by the head but read by an op only up to this cap (txco_drive_too_large). 0 = unlimited. (33554432, 32 MiB)"`
	SignedURLBase                string   `id:"signed-url-base" default:"" desc:"Public origin (scheme://host[:port], no path) that txco://drive/sign mints its URLs on; the web head serves them at /_txc/signed/<token> on every hostname, so any name that reaches it works. Empty derives from --continuation-callback-base-url, then --fqdn / web addr."`
	DriveSweepPeriod             int      `id:"drive-sweep-period" default:"900" desc:"Seconds between drive sweeps on a node running the 'webdav' head: superseded versions and the objects of failed writes are reclaimed, tombstones past --drive-tombstone-retention hard-deleted. Idempotent, so several heads sweeping is wasteful, not wrong. 0 disables. (900)"`
	DriveSweepGrace              int      `id:"drive-sweep-grace" default:"3600" desc:"Seconds an object must be older than before the sweeper may reclaim it — covers a put whose bytes landed before its index row and a reader still streaming a version that was just replaced. (3600)"`
	DriveTombstoneRetention      int      `id:"drive-tombstone-retention" default:"604800" desc:"Seconds a deleted resource's tombstone row stays, so a txco://drive/list since=<modseq> consumer can learn of the deletion; the sweeper hard-deletes older ones. Bytes are reclaimed after --drive-sweep-grace regardless. (604800 = 7d)"`
	WebsocketMaxConns            int      `id:"websocket-max-conns" default:"8192" desc:"Max open WebSocket sessions on this node, all tenants; an upgrade past the cap answers 503 with Retry-After. A memory guard, not a throughput limit: an idle session costs ~50 KB and no processor time, and message work is bounded by tenant admission. Keep it under the process's file-descriptor limit and under any front proxy's connection cap. 0 disables the cap. Needs 'websocket' (with 'web') in --personalities. (8192)"`
	WebsocketMaxConnsPerTenant   int      `id:"websocket-max-conns-per-tenant" default:"2048" desc:"Max open WebSocket sessions per tenant on this node; past it an upgrade answers 503 with Retry-After. Stops one tenant from taking a node's whole session budget. 0 disables the cap. (2048)"`
	WebsocketMaxMessageBytes     int      `id:"websocket-max-message-bytes" default:"262144" desc:"Max bytes of one complete WebSocket message, inbound (a larger one closes the session with 1009) and outbound (txco://websocket/send answers txco_websocket_message_too_large). Text and binary alike; a stack may lower it per session at accept. (262144, 256 KiB)"`
	WebsocketInboundQueue        int      `id:"websocket-inbound-queue" default:"16" desc:"Complete inbound messages a session may hold while its previous message is still running (runs are strictly serialized per session). When the queue is full the session is closed with 1013 rather than growing memory. (16)"`
	WebsocketRunTimeout          string   `id:"websocket-run-timeout" default:"60s" desc:"Deadline for the processor run one inbound WebSocket message makes (<stack>/_websocket/0). A late result is discarded; the session stays open. (60s)"`
	WebsocketIdleTimeout         string   `id:"websocket-idle-timeout" default:"5m" desc:"Close a session (1000) after this long without an application message in either direction; ping/pong does not count. A stack may set a longer one per session at accept, up to --websocket-max-idle-timeout. (5m)"`
	WebsocketMaxIdleTimeout      string   `id:"websocket-max-idle-timeout" default:"1h" desc:"Ceiling for a per-session idle_timeout a stack asks for at accept. (1h)"`
	WebsocketPingInterval        string   `id:"websocket-ping-interval" default:"25s" desc:"Chassis-level liveness: a ping frame every interval, and the pong must arrive before the next; a silent peer is closed (1011). Keep it under any proxy's HTTP idle timeout on the path (fly-proxy: ~60s). (25s)"`
	WebsocketWriteTimeout        string   `id:"websocket-write-timeout" default:"10s" desc:"Deadline for one outbound write (a txco://websocket/send or reply). A peer that does not read within it has its session closed (1011) — messages are never silently dropped. (10s)"`
	WebsocketDrainTimeout        string   `id:"websocket-drain-timeout" default:"5s" desc:"On shutdown every session is closed with 1001 (going away); this bounds how long the chassis waits for those close handshakes. (5s)"`
	WebsocketRelay               string   `id:"websocket-relay" default:"" desc:"Cross-node delivery for txco://websocket/send, reply and close: empty (default) = a session is reachable only on the node holding its socket. Names a registered relay backend; needs a shared --kvstore (redis), which records the node owning each session. ()"`
	IPPStore                     string   `id:"ipp-store" default:"sqlite" desc:"Backend for the ipp personality's JOB store (one row per print job: receiving → committed → delivered): {sqlite}. sqlite is the bundled SQLite file at --ipp-db-path; a downstream overlay can register a shared backend so a job committed on one node is handed to the bus by another. Documents are never stored here — they stream into the file CAS. (sqlite)"`
	IPPDBPath                    string   `id:"ipp-db-path" default:"./chassis/data/ipp.db" desc:"Path to the bundled ipp job store SQLite file (used by --ipp-store=sqlite; ignored by other backends). Its parent dir is created on demand. (./chassis/data/ipp.db)"`
	IPPFormats                   []string `id:"ipp-formats" default:"application/pdf" desc:"Document formats every printer advertises (document-format-supported) and accepts, first = the default. A job's declared format must be listed AND the first bytes of the document must look like it. application/postscript is never accepted whatever is listed. (application/pdf)"`
	IPPMaxJobBytes               int      `id:"ipp-max-job-bytes" default:"268435456" desc:"Size cap for one printed document. Bytes stream into the file CAS, so this bounds a job, not memory. Over it the job is refused with client-error-request-entity-too-large and nothing is stored. (268435456 = 256 MiB)"`
	IPPMaxInflight               int      `id:"ipp-max-inflight" default:"4" desc:"Max documents one tenant may be uploading at once; over it a new job gets server-error-busy (print clients retry). 0 = unlimited. (4)"`
	IPPInsecureAuth              bool     `id:"ipp-insecure-auth" default:"false" desc:"Accept Basic authentication on a plaintext request. Off (default), a request must arrive over TLS — the chassis's own --web-tls-addr, or a front proxy that sets X-Forwarded-Proto: https. Dev only. (false)"`
	IPPAnonymousAttributes       bool     `id:"ipp-anonymous-attributes" default:"false" desc:"Answer Get-Printer-Attributes without credentials. Off (default): NOTHING on an ipp host is answered without the password — a bare request is challenged (401) before its body is read, and the client authenticates and asks again. Turn on only if a print client cannot add the printer otherwise (some query a printer's capabilities while ADDING it, before they have a password to offer); the anonymous answer carries nothing tenant-specific (the fixed capability set + the printer label from the URL), but it does confirm that a printer exists at that hostname. Every other operation always authenticates. (false)"`
	IPPPollInterval              int      `id:"ipp-poll-interval" default:"1" desc:"Seconds between dispatcher passes that hand committed print jobs to the bus. A freshly committed job also nudges the dispatcher, so this is the retry cadence, not the delivery latency. (1)"`
	IPPDispatchTimeout           int      `id:"ipp-dispatch-timeout" default:"10" desc:"Seconds the dispatcher waits for the bus to ACCEPT one job's envelope before releasing it for a later pass. It never waits for the stack's run. (10)"`
	IPPLeaseStaleAfter           int      `id:"ipp-lease-stale-after" default:"600" desc:"Seconds a dispatcher's lease on a committed job may sit before another node may take it — crash recovery for a node that died mid-handoff. (600)"`
	IPPMaxAttempts               int      `id:"ipp-max-attempts" default:"20" desc:"Delivery attempts before a committed job the bus never accepted is marked failed. 0 = retry forever. (20)"`
	IPPReceiveTimeout            int      `id:"ipp-receive-timeout" default:"600" desc:"Seconds a job may sit in 'receiving' with no document (a Create-Job never followed by Send-Document, or an upload whose node died) before it is failed. Advertised as multiple-operation-time-out. (600)"`
	IPPRetention                 int      `id:"ipp-retention" default:"604800" desc:"Seconds to keep terminal (delivered/canceled/failed) job rows before the dispatcher purges them. The documents stay in the CAS. (604800 = 7d)"`
	IPPWireDebug                 bool     `id:"ipp-wire-debug" default:"false" desc:"Log every IPP request's operation and the NAMES of the attributes it carried and requested (never their values: job names and user names are private). For recording what a print client actually sends. (false)"`
	WebTLSSelfSigned             bool     `id:"web-tls-self-signed" default:"false" desc:"Serve --web-tls-addr with a self-signed certificate (minted once under --web-tls-self-signed-cert-dir; loopback + the dev-local hostname patterns + --web-tls-self-signed-hosts) instead of the ACME certificate manager. Dev only: a print client or browser must be told to trust it. (false)"`
	WebTLSSelfSignedCertDir      string   `id:"web-tls-self-signed-cert-dir" default:"./chassis/data" desc:"Directory holding web-selfsigned.crt/.key for --web-tls-self-signed; kept across restarts so the certificate is trusted once. (./chassis/data)"`
	WebTLSSelfSignedHosts        []string `id:"web-tls-self-signed-hosts" default:"" desc:"Extra DNS names for the --web-tls-self-signed certificate, beyond localhost, *.localhost and *.local.thanks.computer."`
	...
	WebMaxBodyBytes              int      `id:"web-max-body-bytes" default:"31457280" desc:"Max web-inlet request-body size in bytes; a larger body is rejected with 413 before it is buffered into the envelope (bounds unauthenticated memory use, since the read happens ahead of admission/rate-limiting). 0 disables the cap (embedder opt-out). (31457280, 30 MiB)"`
	LLMUpstreamURL               string   `id:"llm-upstream-url" default:"https://api.anthropic.com" desc:"Base URL the AI-gateway inlet (POST /v1/messages) forwards to after the tenant's _llm stack runs. Point at a local fake for dev/testing. A stack may override per request via _txc.llm.upstream.url; the chassis egress policy applies to both. (https://api.anthropic.com)"`
	LLMContextMaxTokens          int      `id:"llm-context-max-tokens" default:"2000" desc:"Hard cap on estimated tokens (bytes/4) of _txc.llm.context items the AI-gateway inlet injects as system blocks per request, guard block included; items beyond the budget are dropped and recorded in _txc.llm.context_result. Context injection is enabled only when BOTH this and --llm-context-max-items are positive; 0 disables injection entirely. (2000)"`
	LLMContextMaxItems           int      `id:"llm-context-max-items" default:"8" desc:"Hard cap on the number of _txc.llm.context items the AI-gateway inlet injects per request. Context injection is enabled only when BOTH this and --llm-context-max-tokens are positive; 0 disables injection entirely. (8)"`
...
func Load() (Config, error) {
	...
	// loop-timeout must parse and fit under op-timeout-max: a LOOP rule
	// with no WITH timeout inherits it, and the dispatch ceiling would
	// otherwise reject every such rule.
	loopTimeoutDur, err := time.ParseDuration(fmt.Sprintf("%v", config.LoopTimeout))
	if err != nil {
		log.Fatalf("unable to parse loop-timeout %s", config.LoopTimeout)
	}
