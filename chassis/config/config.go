...
type Config struct {
	...
	Personalities                string   `id:"personalities" default:"cron,tcp,web,admin" desc:"Head types to start. Comma delimited. {cron,tcp,web,admin,lmtp,sweep,dns,mailmap,scheduled,imap} (cron,tcp,web,admin)"`
	...
	DNSIMAPSPort                 int      `id:"dns-imaps-port" default:"0" desc:"Port advertised in synthesized RFC 6186 _imaps._tcp SRV records for delegated zones — at the apex and every per-stack host, target = the name itself; the default-suffix wildcard zone advertises imap.<suffix> (one target under the wildcard certificate) — so mail clients that look up SRV (Thunderbird, K-9, Evolution) find the IMAP server without typing it. 0 (default) emits no SRV. Set 993 only when an IMAPS front door serves every hostname of the zone. (0)"`
