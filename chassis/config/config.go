...
type Config struct {
	...
	IMAPStore                    string   `id:"imap-store" default:"sqlite" desc:"Backend for the IMAP mailbox index (accounts, mailboxes, message rows; bytes live in the content store): {sqlite}. sqlite is the bundled SQLite file at --imap-db-path — never the runtime DB. (sqlite)"`
	IMAPDBPath                   string   `id:"imap-db-path" default:"./chassis/data/imap.db" desc:"Path to the bundled IMAP index SQLite file (used by --imap-store=sqlite). Its parent dir is created on boot. (./chassis/data/imap.db)"`
