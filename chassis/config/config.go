...
type Config struct {
	...
	IPPAnonymousAttributes       bool     `id:"ipp-anonymous-attributes" default:"true" desc:"Answer Get-Printer-Attributes without credentials. Print clients query a printer's capabilities while ADDING it, before they have asked the user for a password; the answer carries nothing tenant-specific (the fixed capability set + the printer label from the URL). Every other operation always authenticates. (true)"`
