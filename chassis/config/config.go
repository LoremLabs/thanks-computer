...
type Config struct {
	...
	CronQueue                    string   `id:"cron-queue" default:"local" desc:"Cron dispatch queue backend: {local}. local (default) is an in-process channel + worker pool; an out-of-tree backend (e.g. nats) self-registers and is selected here. Single-node behavior is unchanged with local."`
	...
	VectorPostgresDSN            string   `id:"vector-postgres-dsn" default:"" desc:"When set, the txco://vector/* ops use a shared PostgreSQL + pgvector store at this DSN (HA / multi-node) instead of the bundled sqlite-vec file. The pgx driver must be registered by the build (overlay/production); the open-core chassis ships SQLite only. Empty = bundled vector-db-path. (empty)"`
