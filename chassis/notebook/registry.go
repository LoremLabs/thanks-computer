...
// affect existing backends (same posture as imap.Config / scheduled.Config).
type Config struct {
	// DBPath is the bundled SQLite backend's file path (--notebook-db-path).
	DBPath string
}
...
// Postgres backend the same way so an append on one node is readable on
// every other.
...
// listing what is available (so a misconfigured --notebook-store fails
// loudly).
func Open(name string, cfg Config) (*Store, error) {
	...
	if !ok {
		...
		return nil, fmt.Errorf("notebook: unknown store %q (available: %v)", name, avail)
	}
	return c(cfg)
}
