-- Postgres mirror of sqlite/auth/0005. The SQL below is identical: no BLOB
-- and no FOREIGN KEY clause to translate, and the partial expression index
-- is native to both engines. Timestamps stay RFC3339 TEXT and booleans
-- INTEGER 0/1 so the Go scan code is one path (see 0001_init.sql).
--
